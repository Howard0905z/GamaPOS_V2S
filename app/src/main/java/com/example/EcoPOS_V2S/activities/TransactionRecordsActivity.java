...
import com.example.EcoPOS_V2S.adapters.TransactionsAdapter;
...
public class TransactionRecordsActivity extends AppCompatActivity implements Serializable {
    ...
    private void fetchTransactions(String date) {
        ...
        String url = EnvironmentConfig.getOrdersWithDateUrl(date);
                    ...
                    try {
                        ...
                        for (int i = 0; i < dataArray.length(); i++) {
                            ...
                            TransactionRecord transaction = new TransactionRecord(
                                    ...
                                    transactionJson.optString("invalidated_by", "N/A"),
                                    transactionJson.optString("invalidated_reason", "N/A")
                            );
    ...
    private void fetchRevenue(String date) {
        ...
        String token = sharedPreferences.getString("jwt_token", "");
        OkHttpClient client = new OkHttpClient();
        String url = EnvironmentConfig.getRevenueReportUrl(date);
        Request request = new Request.Builder()
                .url(url)
                .addHeader("Authorization", "Bearer " + token)
                .addHeader("Accept", "application/json")
