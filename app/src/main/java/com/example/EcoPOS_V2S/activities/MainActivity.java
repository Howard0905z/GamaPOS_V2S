...
import com.example.EcoPOS_V2S.adapters.CategoryAdapter;
import com.example.EcoPOS_V2S.models.CartItem;
import com.example.EcoPOS_V2S.models.Category;
import com.example.EcoPOS_V2S.models.Product;
import com.example.EcoPOS_V2S.models.ResponseWrapper;
import com.example.EcoPOS_V2S.models.SubCategory;
...
public class MainActivity extends AppCompatActivity {
    ...
    private void initializeCart() {
        ...
        double maxPercentageDiscount = dataManager.getMaxPercentageDiscount();
        double maxFixedDiscount = dataManager.getMaxFixedDiscount();
    ...
    private void fetchProductBySKU(String sku) {
        networkManager.fetchProductBySKU(sku, new NetworkManager.ApiCallback() {
            @Override
            public void onSuccess(String response) {
                try {
                    JSONArray dataJsonArray = new JSONObject(response).getJSONArray("data");
    ...
    private void fetchCategories() {
        networkManager.fetchCategories(new NetworkManager.ApiCallback() {
            @Override
            public void onSuccess(String response) {
                Type responseType = new TypeToken<ResponseWrapper<List<Category>>>(){}.getType();
                ResponseWrapper<List<Category>> responseWrapper = new Gson().fromJson(response, responseType);
                ...
                runOnUiThread(() -> {
                    ...
                    CategoryAdapter adapter = new CategoryAdapter(MainActivity.this, categoryObjects, 
                            category -> onMainCategoryClicked((Category) category), false);
    ...
    private void fetchAllProducts() {
        networkManager.fetchAllProducts(new NetworkManager.ApiCallback() {
            @Override
            public void onSuccess(String response) {
                try {
    ...
    private void fetchProducts(SubCategory subCategory) {
        ...
        Request request = new Request.Builder()
                .url(EnvironmentConfig.getProductsUrl())
